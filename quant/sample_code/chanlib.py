...
def run_chan_analysis(stock_code,end_date,stock_days,x_jizhun,stock_frequency,chanK_flag):
    ...
    handle_box_down=matplt.patches.Polygon(vertices_down,color='g',zorder=1)
    handle_box_even=matplt.patches.Polygon(vertices_even,color='k',zorder=1)

    ax1.add_patch(handle_box_up)
    ax1.add_patch(handle_box_down)
    ax1.add_patch(handle_box_even)

    handle_line_up=matplt.lines.Line2D(pad_X,line_up,color='k',linestyle='solid',zorder=0) 
    handle_line_down=matplt.lines.Line2D(pad_X,line_down,color='k',linestyle='solid',zorder=0) 
    ...
    #         dt = chanK['enddate'][biIdx[i]]
              # 缠论k线
            _dt = chanK.index[biIdx[i]] if chanK_flag else chanK['enddate'][biIdx[i]]
    #         print k_data['high'][dt], k_data['low'][dt]
            time_long = np.long(time.mktime((_dt+datetime.timedelta(hours=8)).timetuple())*1000000000)
    ...
    #         dt = chanK['enddate'][biIdx[i]]
              # 缠论k线
            _dt = chanK.index[xdIdxs[i]] if chanK_flag else chanK['enddate'][xdIdxs[i]]
    #         print k_data['high'][dt], k_data['low'][dt]
            time_long = np.long(time.mktime((_dt+datetime.timedelta(hours=8)).timetuple())*1000000000)
    #         print x_date_list.index(time_long) if time_long in x_date_list else 0
            if fenType == 1:
